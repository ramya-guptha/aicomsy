...
class NcrReport(models.Model):
    ...
    discipline_id = fields.Many2one('x.ncr.discipline', string='Discipline', required=True)
    supplier_name_id = fields.Many2one('res.partner', domain=[('supplier_rank', '>', 0)], string='Supplier Name', required=True)
    purchase_order_no = fields.Char(string='Purchase Order No.', required=True)
